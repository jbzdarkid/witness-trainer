...
std::unordered_map<int32_t, const char*> Entities::ALL_ENTITIES;
std::unordered_map<int32_t, const char*> Entities::COUNTED_PANELS;
std::unordered_map<int32_t, const char*> Entities::UNCOUNTED_PANELS;
Entities::Constructor Entities::_construct;

std::string Entities::NameOf(int id) {
	if (id == -1) return "";
	auto search = ALL_ENTITIES.find(id);
	if (search == ALL_ENTITIES.end()) return "(invalid)";
...
Entities::Constructor::Constructor() {
	...
	UNCOUNTED_PANELS = {
		...
		{0x33638, "Easter Egg Ending Lightswitch"},
	...
	ALL_ENTITIES = {};
    for (const auto& it : COUNTED_PANELS) ALL_ENTITIES.insert(it);
    for (const auto& it : UNCOUNTED_PANELS) ALL_ENTITIES.insert(it);

	ALL_ENTITIES[0X1C343] = "Invisible desert laser redirect 1";
	ALL_ENTITIES[0X1C344] = "Invisible desert laser redirect 2";
}
